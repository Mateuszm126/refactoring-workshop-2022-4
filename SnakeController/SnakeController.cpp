...
namespace Snake
{
    ...
    Controller::Controller(IPort &p_displayPort, IPort &p_foodPort, IPort &p_scorePort, std::string const &p_config)
        : m_displayPort(p_displayPort),
          ...
          m_paused(false),
          m_snakeSegments(std::make_unique<SnakeSegments>())
    {
        ...
        if (w == 'W' and f == 'F' and s == 'S')
        {
            ...
            while (length--)
            {
                Segment seg;
                istr >> seg.x >> seg.y;
                //m_snakeSegments->m_segmentsPushBask(seg);
    ...
    void Controller::updateSegmentsIfSuccessfullMove(Segment const &newHead)
    {
        ...
        // if (m_snakeSegments->isSegmentAtPosition(newHead.x, newHead.y) or m_snakeSegments->isSegmentAtPosition(newHead.x, newHead.y))
        {
    ...
    void Controller::updateFoodPosition(int x, int y, std::function<void()> clearPolicy)
    {
        ...
        // if (m_snakeSegments->isSegmentAtPosition(x, y) || m_snakeSegments->isSegmentAtPosition(x, y))
        {
            m_foodPort.send(std::make_unique<EventT<FoodReq>>());
            return;
        }
